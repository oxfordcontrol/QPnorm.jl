...
function find_feasible_point(A, b, r)
	...
	@assert maximum(A*x_min - b) <= 1e-16 string("Min-radius solution returned by IPOPT violates constraints by:", maximum(A*x_min - b))
	...
	@assert maximum(A*x_max - b) <= 1e-16 string("Max-radius solution returned by IPOPT violates constraints by:", maximum(A*x_max - b))
...
function optimize_radius(A, b, r; minimize=true)
	n = size(A, 2)
	model = JuMP.Model()
    setsolver(model, IpoptSolver(max_iter=500, bound_relax_factor=0.0, constr_viol_tol=1e-18))#, print_level=0))
...
function solve_ipopt(P, q, A, b, r)
	n = size(P, 1)
	model = JuMP.Model()
    setsolver(model, IpoptSolver(max_iter=3000))#, print_level=0))
	@variable(model, x[1:n])
